...
ggplot(gbbo_age, aes(x = Age, y = percentage)) +
  geom_bar(stat = "identity") +
  facet_grid(. ~ Technical.Rank) + 
...
**Interaction is indeed warranted (results from ANOVA significant)**
...
m <- clm(Technical.Rank ~ Gender * Age, data = gbbo_filter, link = "logit")

m0 <- clm(Technical.Rank ~ Gender + Age, data = gbbo_filter, link = "logit")
...
-   Use `ggemmeans` to create a figure showing the interaction between `Gender` and `Age` as a function of rank.
    ...
    ```{r}
    interact <- ggemmeans(m, terms = c("Age", "Gender"), type = "fixed")
    interact
    ...
    ggplot(interact,
      ...
      labs(title = "Predicted probabilities from the interaction model",
           x = "Age",
           y = "Percentage")
    ```
...
**The results of the brm and clm model are near identical with respect to the estimate coefficients**
