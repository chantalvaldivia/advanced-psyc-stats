...
In an intercept only model with bayes we don't have the priors yet and are therefore uninformed
...
Standard deviations cant be negative? So theoretically yes these are fine but I'm not sure I'd be able to tell if a standard deviation was all that implausible given my data. Maybe if it were abnormally large and that doesn't fit with my data I'd take a second glance
...
We are making a tibble (so a dataframe) by specifying the intercept (beta0) and st dev to make a bunch of sample distributions?
...
It seems like the second prior predictive (centered at 8, etc) is much more consistent than the first, so I'd go with that one. The prior predictive check with the first set of distributions just has way too much variance for my liking.
...
Prior predictive checks use priors to simulate data

Posterior predictive checks how well the model fits observed data (checking simulated data)
...
Both posterior predictive checks seem adequate, but I'm more convinced by the female model than the male model – and this is particularly noticeable the first graph we made?
