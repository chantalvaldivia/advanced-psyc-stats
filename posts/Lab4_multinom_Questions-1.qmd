...
voter_data$voter_category <- ordered(voter_data$voter_category, levels=c("rarely/never", "sporadic", "always"))
...
# Enter code
plot_race <- ggplot(voter_data, aes(x = race, y = voter_category))+
  geom_col(aes(fill = voter_category), width = 0.7) 

plot_race + coord_flip()

```

```{r}
# Enter code
plot_gender <- ggplot(voter_data, aes(x = gender, y = voter_category))+
  geom_col(aes(fill = voter_category), width = 0.7) 

plot_gender + coord_flip()
