...
Below is some code that fits a model of "reading age" (`R_AGE`)
predicted by hours of practice (`hrs_week`). Line 2 then gets the
'fitted' values from the model and adds them as a new column to the
dataset, called `pred_lm`. The fitted values are what the model predicts
for every individual observation (every individual toy in our dataset).
...
**One thing I notice about the lines is they all appear to have the same
slope (though the length of the lines differ) and that we are not
representing the data well at all with this model**
...
**Okay something I notice in the first chunk here below is that the
slope still appears to be the same but now we've moved up along the
y-axis and actually meeting the observed values in the 2 dimensional
space, but still not a good model. The second chunk of code produces the
exact same results we saw in the first chunk, which is interesting.
Finally the third model fits the data way better, the slope actually
varies by toy and the line of best fit meets all the observed values.
This third model is the best so far.**
...
#Attempt N/A because we copied and pasted code as instructed
```

**Okay something I notice in the first chunk here below is that the
slope still appears to be the same but now we've moved up along the
y-axis and actually meeting the observed values in the 2 dimensional
space, but still not a good model.**

**'The second chunk of code produces the exact same results we saw in
the first chunk, which is interesting.**
...
# My answer didn't differ in code because here we were just copying and pasting but mostly I think I understood what was going on. I didn't catch that in the second model we were modelling distributions though. 
...
#A3 = where the black line cuts the y axis (at x=0)
...
-   Okay for this one below I think it is A2?$\sigma_{\varepsilon}$

-   And for this one I think it is A4? $b_{1}$

-   And this is one is A1$\sigma_{0}$

-   And this one is A3$\gamma_{00}$

Okay I got A4, A3, A2, and A1 correct woohoo
...
table(efdat) # question 1
(unique(efdat$PID)) #question 2


efdat |> # question 3: wouldn't have known how to this on my own
  count(PID)

head(table(efdat$PID, efdat$audio)) # question 4

```

**Looks like there are 30 participants in the data**
...
-   **The outcome of interest would be SDMT scores**\

-   What variables are we seeking to investigate in terms of their
    impact on the outcome?\
    **We interested in how audio variables (no music, white noise,
    music) affect SDMT scores (with an interaction for headphones)**

-   What are the units of observations?\
    **I was looking for units of measurement like decibels or \# correct
    on SDMT but the answer seems to be individual trials**

-   Are the observations clustered/grouped? In what way?

-   **Observations are grouped by individual (so that would be something
    like a random effect) and also the music conditions are clustered
    around use of noise canceling headphones or not**\

-   What varies *within* these clusters?\
    **See above, audio source**

-   What varies *between* these clusters?

-   **Use of headphones or not**
...
#efdat <- efdat %>%
  #mutate(audio = factor(audio,
                           #levels = c("no audio, speakers")),
  #mutate(headphones = factor(headphones),
         #levels = c("speakers"))
...
lmer(SDMT ~ audio*headphones + (1 | PID), data = efdat)
```
...
We now have a model, but we don't have any p-values or confidence
intervals or anything - i.e. we have no inferential criteria on which to
draw conclusions. There are a whole load of different methods available
for drawing inferences from multilevel models, which means it can be a
bit of a never-ending rabbit hole. For now, we'll just use the 'quick
and easy' approach provided by the **lmerTest** package seen in the
lectures.
...
refit <- lmerTest::lmer(SDMT ~ audio*headphones + (1 | PID), data = efdat)
refit # i actually don't see p values here?

summary(refit) # okay now looking at the answer key i had to call for summary here, makes sense!
```
...
Now we have some p-values and a plot, try to create a short write-up of
...
```{r}
#We were interested in the effects of audio and headphone usage on SDMT. To test these effects, we used a linear mixed effects model (where fixed effects were audio type (no audio/white noise/music) whereby we were interested in the interaction between audio type and the source (headphones/speakers). The random effects were the participants.

#We fitted models using lme4 package in R

#It seems indeed that listneing to music affects SDMT scores (as opposed to not listening music). There was an interaction effect of audio source though, such that those who listened to white noise with headphones – their performance improved.

#30 participants were recruited to complete 5 trials each, but not all participants were able to complete all 15 trials.
```
